...
class DonviController extends Controller
{
    ...
    public function actionIndex(){
        if (Yii::$app->user->can('donvi/index')) {
    ...
    public function actionImport()
    {
        ...
        {
            if (Yii::$app->user->can('donvi/import')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {  
                    ...
                    if ($model->file && $model->validate()) {     
                        ...
                        for($i = 2; $i <= $Totalrow; $i++){
                            $info = new DonVi();
                            $info->madonvi = (string)$sheetData->getCell('A'.$i)->getValue();
                            $info->tendonvi = (string)$sheetData->getCell('B'.$i)->getValue();
                            $info->tuyen = $sheetData->getCell('C'.$i)->getValue();
                            $info->hang = $sheetData->getCell('D'.$i)->getValue();
                            $info->id_huyen = $sheetData->getCell('E'.$i)->getValue();
                            $info->phienban = $sheetData->getCell('F'.$i)->getValue();
                            $info->id_parent = (string)$sheetData->getCell('G'.$i)->getValue();
                            $info->phone = (string)$sheetData->getCell('H'.$i)->getValue();
                            $info->status = $sheetData->getCell('I'.$i)->getValue();
                ...
                return Json::encode(array('status' => true, 'title' => 'Import đơn vị cơ sở KCB', 'data' => $this->renderAjax('_import', ['model' => $model])));
            }
            else{
    ...
    public function actionCreate()
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('donvi/create')) {
                $model = new DonVi();
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                    $transaction = \Yii::$app->db->beginTransaction();          
                    try {
                    ...
                    return Json::encode(array('status' => true, 'title' => 'Thêm đơn vị cơ sở KCB', 'data' => $this->renderAjax('_create', ['model' => $model])));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
    ...
    public function actionUpdate($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('donvi/update')) {
                $model = DonVi::findOne(['madonvi' => $id]);
                if($model){
                    if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                        $transaction = \Yii::$app->db->beginTransaction();          
                        try {
                        ...
                        return Json::encode(array('status' => true, 'title' => 'Sửa đơn vị cơ sở KCB', 'data' => $this->renderAjax('_update', ['model' => $model])));
                    }
                }
                else{
                    return Json::encode(array('status' => false, 'hideModal' => true, 'message' => 'Thật xin lỗi! Đơn vị này không còn tồn tại!'));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
