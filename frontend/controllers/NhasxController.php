...
class NhasxController extends Controller
{
    ...
    public function actionIndex(){
        if (Yii::$app->user->can('nhasx/index')) {
            $searchModel = new NhaSxSearch();
    ...
    public function actionImport()
    {
        ...
        {
            if (Yii::$app->user->can('nhasx/import')) {
                ...
                return Json::encode(array('status' => true, 'title' => 'Import nhà sản xuất', 'data' => $this->renderAjax('_import', ['model' => $model])));
            }
            else{
    ...
    public function actionCreate()
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('nhasx/create')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                    ...
                    return Json::encode(array('status' => true, 'title' => 'Thêm nhà sản xuất', 'data' => $this->renderAjax('_create', ['model' => $model])));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
    ...
    public function actionUpdate($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('nhasx/update')) {
                $model = NhaSanXuat::findOne($id);
                if($model){
                    if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                        $transaction = \Yii::$app->db->beginTransaction();          
                        try {
                        ...
                        return Json::encode(array('status' => true, 'title' => 'Sửa nhà sản xuất', 'data' => $this->renderAjax('_update', ['model' => $model])));
                    }
                }
                else{
    ...
    public function actionDelete($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('nhasx/delete')) {
                $model = NhaSanXuat::findOne($id);
                if($model){
