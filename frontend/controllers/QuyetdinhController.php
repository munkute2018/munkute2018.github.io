...
class QuyetdinhController extends Controller
{
    ...
    public function actionIndex(){
        if (Yii::$app->user->can('quyetdinh/index')) {
    ...
    public function actionImport()
    {
        ...
        {
            if (Yii::$app->user->can('quyetdinh/import')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {  
                    ...
                    if ($model->file && $model->validate()) {     
                        ...
                        for($i = 2; $i <= $Totalrow; $i++){
                            $info = new Dmthongtuhis();
                            $info->name = (string)$sheetData->getCell('A'.$i)->getValue();
                            $info->bhxh = $sheetData->getCell('B'.$i)->getValue();
                            $info->posted = $sheetData->getCell('C'.$i)->getValue();
                            $info->status = $sheetData->getCell('D'.$i)->getValue();
                            if (!$info->validate() || !$info->saveThongTu()) {
    ...
    public function actionCreate()
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('quyetdinh/create')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                    ...
                    return Json::encode(array('status' => true, 'title' => 'Thêm danh mục quyết định', 'data' => $this->renderAjax('_create', ['model' => $model])));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
    ...
    public function actionUpdate($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('quyetdinh/update')) {
                $model = Dmthongtuhis::findOne(['id' => $id]);
                $model->posted = date('d/m/Y', $model->posted_at);
                if($model){
                    if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                        $transaction = \Yii::$app->db->beginTransaction();          
                        try {
                        ...
                        return Json::encode(array('status' => true, 'title' => 'Sửa danh mục quyết định', 'data' => $this->renderAjax('_update', ['model' => $model])));
                    }
                }
                else{
                    return Json::encode(array('status' => false, 'hideModal' => true, 'message' => 'Thật xin lỗi! Quyết định này không còn tồn tại!'));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
