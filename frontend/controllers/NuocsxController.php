...
use common\models\NuocSxSearch;
...
class NuocsxController extends Controller
{
    ...
    public function actionIndex(){
        if (Yii::$app->user->can('nuocsx/index')) {
            $searchModel = new NuocSxSearch();
    ...
    public function actionImport()
    {
        ...
        {
            if (Yii::$app->user->can('nuocsx/import')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {  
                    ...
                    if ($model->file && $model->validate()) {     
                        ...
                        for($i = 2; $i <= $Totalrow; $i++){
                            $info = new NuocSanXuat();
                            $info->id = $sheetData->getCell('A'.$i)->getValue();
                            $info->ten_nsx = (string)$sheetData->getCell('B'.$i)->getValue();
                            $info->status = 1;
                ...
                return Json::encode(array('status' => true, 'title' => 'Import nước sản xuất', 'data' => $this->renderAjax('_import', ['model' => $model])));
            }
            else{
    ...
    public function actionCreate()
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('nuocsx/create')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                    ...
                    return Json::encode(array('status' => true, 'title' => 'Thêm nước sản xuất', 'data' => $this->renderAjax('_create', ['model' => $model])));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
    ...
    public function actionUpdate($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('nuocsx/update')) {
                $model = NuocSanXuat::findOne($id);
                if($model){
                    if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                        $transaction = \Yii::$app->db->beginTransaction();          
                        try {
                        ...
                        return Json::encode(array('status' => true, 'title' => 'Sửa nước sản xuất', 'data' => $this->renderAjax('_update', ['model' => $model])));
                    }
                }
                else{
                    return Json::encode(array('status' => false, 'hideModal' => true, 'message' => 'Thật xin lỗi! Nhà sản xuất này không còn tồn tại!'));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
    ...
    public function actionDelete($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('nuocsx/delete')) {
                $model = NuocSanXuat::findOne($id);
                if($model){
