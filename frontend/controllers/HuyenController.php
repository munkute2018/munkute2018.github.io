...
class HuyenController extends Controller
{
    ...
    public function actionIndex(){
        if (Yii::$app->user->can('huyen/index')) {
            $searchModel = new HuyenSearch();
    ...
    public function actionImport()
    {
        ...
        {
            if (Yii::$app->user->can('huyen/import')) {
                ...
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {  
                    ...
                    if ($model->file && $model->validate()) {     
                        ...
                        for($i = 2; $i <= $Totalrow; $i++){
                            $info = new Huyen();
                            $info->huyen = (string)$sheetData->getCell('A'.$i)->getValue();
                            $info->status = $sheetData->getCell('B'.$i)->getValue();
                ...
                return Json::encode(array('status' => true, 'title' => 'Import danh mục huyện', 'data' => $this->renderAjax('_import', ['model' => $model])));
            }
            else{
    ...
    public function actionCreate()
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('huyen/create')) {
                $model = new Huyen();
                if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                    $transaction = \Yii::$app->db->beginTransaction();          
                    try {
                    ...
                    return Json::encode(array('status' => true, 'title' => 'Thêm danh mục huyện', 'data' => $this->renderAjax('_create', ['model' => $model])));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
    ...
    public function actionUpdate($id=0)
    {
        if(isset($_SERVER['HTTP_REFERER']))
        {
            if (Yii::$app->user->can('huyen/update')) {
                $model = Huyen::findOne(['id' => $id]);
                if($model){
                    if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                        $transaction = \Yii::$app->db->beginTransaction();          
                        try {
                        ...
                        return Json::encode(array('status' => true, 'title' => 'Sửa danh mục huyện', 'data' => $this->renderAjax('_update', ['model' => $model])));
                    }
                }
                else{
                    return Json::encode(array('status' => false, 'hideModal' => true, 'message' => 'Thật xin lỗi! Huyện này không còn tồn tại!'));
                }
            }
            else{
                return Json::encode(array('status' => false, 'hideModal' => false, 'message' => 'Bạn không có quyền sử dụng tính năng này! Liên hệ người quản trị để cấp quyền cho bạn hoặc đăng nhập với tài khoản khác!'));
